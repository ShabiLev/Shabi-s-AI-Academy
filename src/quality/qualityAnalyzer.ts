import { computeReleaseStatus, hasSevereAccessibilityViolation, isCoverageBelowThreshold } from './qualityStatus'
import { gateNames, type AnalyzerSummary, type BilingualText, type GateName, type QualityReport, type ReleaseStatus } from './types'
 ...
 */
export function analyzeQuality(report: QualityReport, checklistComplete: boolean): AnalyzerSummary {
  const overallStatus = computeReleaseStatus(report, checklistComplete)
  const failedGates = gateNames.filter((name) => report.gates[name].status === 'failed')
  const warningGates = gateNames.filter((name) => report.gates[name].status === 'warning')

  const recommendedActions: BilingualText[] = []
  const likelyAffectedAreas: BilingualText[] = []
  const addArea = (area: BilingualText) => {
    if (!likelyAffectedAreas.some((a) => a.en === area.en)) likelyAffectedAreas.push(area)
  }
  ...
  if (report.gates.unitTests.status === 'failed') {
    ...
    addArea({ en: 'Application logic (Vitest)', he: 'לוגיקת האפליקציה (Vitest)' })
  }
