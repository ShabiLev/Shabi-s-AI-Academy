...
- Occurred: 2026-07-14T14:02:31.987Z to 2026-07-14T14:19:35.852Z
- Branch: feature/1.4.0-agent-operating-system
- Commit tested: f922b55fc2cd8766beb8de0fa91a1749ce7e7114
- Profile: full
...
- Commands passed: npm run docs:check, npm run aos:check, npm run test:evidence, npm run test:aos, npm run lint, npm run test:run, npm run test:coverage, npm run catalog:check, npm run test:e2e, npm run test:journeys, npm run test:click-audit, npm run test:route-crawl, npm run test:forms, npm run test:overlays, npm run test:responsive:interactions, npm run test:keyboard, npm run test:copy, npm run test:errors, npm run test:ux, npm run test:a11y, npm run quality:collect, npm run quality:system-report, git diff --check
- Commands failed: npm run build, npm run build:pages, npm run test:e2e:full, npm run test:visual, npm run test:performance, npm run test:release-candidate, npm run test:release-candidate:pages, npm run quality:analyze, npm run validate:release
- Heavy local artifacts: `quality/execution/runs/2026-07-14_17-02-31_feature-1-4-0-agent-operating-system/` (ignored), plus copied Playwright, coverage, and quality-generated reports when available.
