...
describe("Shabi's AI Academy", () => {
  ...
  it('redirects authenticated visitors away from Login', async () => {
    ...
    first.unmount()
    renderApp('/login')
    expect(screen.getByRole('heading', { name: 'ברוך שובך, שבי' })).toBeInTheDocument()
  })

  it('uses the correct Hebrew name in the welcome text', async () => {
    const user = userEvent.setup(); renderApp(); await demoLogin(user)
    expect(screen.getByRole('heading', { name: 'ברוך שובך, שבי' })).toBeInTheDocument()
  ...
  it('does not render a language switcher in the global header', async () => {
    const user = userEvent.setup(); renderApp(); await demoLogin(user)
    expect(screen.queryByRole('group', { name: 'בחירת שפה' })).not.toBeInTheDocument()
  })

  it('changes language and direction from Settings', async () => {
    const user = userEvent.setup(); renderApp('/settings'); await demoLogin(user)
  ...
  it('persists the Settings language selection after remount', async () => {
    const user = userEvent.setup(); const first = renderApp('/settings'); await demoLogin(user)
    await user.click(screen.getByRole('radio', { name: /English/ }))
    first.unmount(); renderApp('/settings')
  ...
  it('signs out and returns to Login', async () => {
    const user = userEvent.setup(); renderApp(); await demoLogin(user)
    await user.click(screen.getByRole('button', { name: 'פתיחת תפריט הפרופיל' }))
    await user.click(screen.getByRole('menuitem', { name: 'התנתקות' }))
    expect(screen.getByRole('heading', { name: 'כניסה' })).toBeInTheDocument()
  })

  it('shows Home and Back controls on Lessons', async () => {
    const user = userEvent.setup(); renderApp('/lessons'); await demoLogin(user)
  ...
  it('does not show redundant Home or Back controls on Dashboard', async () => {
    const user = userEvent.setup(); renderApp(); await demoLogin(user)
    expect(screen.queryByRole('button', { name: 'בית' })).not.toBeInTheDocument()
