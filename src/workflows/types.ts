...
export type WorkflowStepResult = "planned" | "executed" | "simulated" | "skipped" | "failed" | "waitingForApproval";
export interface WorkflowTimelineEvent { id: string; nodeId: string; nodeType: WorkflowNodeType; status: "completed" | "waitingForApproval" | "skipped" | "failed"; result?: WorkflowStepResult; timestamp: string; summary: string }
export interface WorkflowRun { id: string; workflowId: string; mode: "mock" | "dryRun"; status: "completed" | "waitingForApproval" | "failed"; realityMode: "Simulated" | "Manual action required"; createdAt: string; completedAt?: string; durationMs: number; input: Record<string,string>; output: string; errors: string[]; deliverableIds: string[]; evidenceIds: string[]; limitations: string[]; nextAction: string; events: WorkflowTimelineEvent[]; validationCodes: string[] }
export interface WorkflowState { schemaVersion: 2; appVersion: string; workflows: Workflow[]; history: WorkflowRun[] }
