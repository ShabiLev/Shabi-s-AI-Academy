import { QUALITY_SCHEMA_VERSION, type GateName, type QualityGates, type QualityReport } from './types'
...
function passedGates(): QualityGates {
  ...
  return Object.fromEntries(names.map((name) => [name, { status: 'passed' }])) as QualityGates
}

/** A fully-passing report, useful as a base that individual tests patch one field at a time. */
export function makeReport(overrides: Partial<QualityReport> = {}): QualityReport {
  return {
    ...
    knownIssues: { openCount: 0, bySeverity: { critical: 0, high: 0, medium: 0, low: 0 } },
