...
export function respondToAssistant(input: string, workspace: AssistantWorkspace): AssistantResponse {
  ...
  if (intent === "defineConcept") {
    const normalized = input.toLocaleLowerCase();
    const term = glossaryTerms.find((item) => [item.id, item.name.en.toLocaleLowerCase(), item.name.he].some((name) => normalized.includes(name))) ?? glossaryTerms.find((item) => item.id === "agent")!;
    return { ...base, type: "informational", text: { he: `${term.name.he}: ${term.definition.he} ${term.availability?.he ?? ""}`.trim(), en: `${term.name.en}: ${term.definition.en} ${term.availability?.en ?? ""}`.trim() } };
  }
  ...
  const actions = { createPrompt: ["createDraftPrompt", "/prompts/new"], createAgent: ["createDraftAgent", "/agents/new"], startMockRun: ["startMockRun", ""], startDryRun: ["startDryRun", ""], openQaReport: ["navigate", "/qa"], exportWorkspace: ["export", ""], showHelp: ["openHelp", "/help"], openRoute: ["search", input] } as const;
