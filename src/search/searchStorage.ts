...
const empty = (): SearchStorageState => ({ schemaVersion: 1, appVersion: "1.5.0-beta.1", queries: [], recentIds: [] });
...
export function parseSearchStorage(value: unknown): SearchStorageState {
  ...
  return { schemaVersion: 1, appVersion: "1.5.0-beta.1", queries: strings(record.queries, MAX_QUERIES), recentIds: strings(record.recentIds, MAX_RECENTS) };
}
