...
const empty = (): SearchStorageState => ({ schemaVersion: 1, appVersion: "2.0.0", queries: [], recentIds: [] });
...
export function parseSearchStorage(value: unknown): SearchStorageState {
  ...
  return { schemaVersion: 1, appVersion: "2.0.0", queries: strings(record.queries, MAX_QUERIES), recentIds: strings(record.recentIds, MAX_RECENTS) };
}
