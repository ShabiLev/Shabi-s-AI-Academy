...
export function SettingsPage() {
  ...
  return <div className="page settings-page">
    ...
    <section className="settings-card" aria-labelledby="progress-settings-title"><h2 id="progress-settings-title">{t('settings.progressTitle')}</h2><p>{t('settings.progressDescription')}</p><button className="danger-button" type="button" onClick={()=>{if(window.confirm(t('settings.resetConfirm'))){resetProgress();setResetMessage(t('settings.resetSuccess'))}}}>{t('settings.resetProgress')}</button>{resetMessage&&<p role="status">{resetMessage}</p>}</section>
  </div>
}
