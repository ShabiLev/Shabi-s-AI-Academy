...
import { getRecommendation, recommendStartingPath, useOnboarding, type ExperienceLevel, type Interest, type MainGoal } from "../onboarding";
...
const interests: Array<{ id: Interest; he: string; en: string }> = [
  { id: "qa", he: "QA ובדיקות תוכנה", en: "QA and software testing" },
  { id: "sql", he: "SQL ונתונים", en: "SQL and data" },
  { id: "product", he: "מוצר וניהול", en: "Product and management" },
  { id: "development", he: "פיתוח", en: "Development" },
  { id: "promptEngineering", he: "Prompt Engineering", en: "Prompt engineering" },
  { id: "agents", he: "Agents", en: "Agents" },
  { id: "automation", he: "אוטומציה", en: "Automation" },
  { id: "research", he: "מחקר AI", en: "AI research" },
];
...
export function OnboardingPage() {
  ...
  const recommendation = step >= 4 ? getRecommendation(selectedArea.recommendationId) : recommendStartingPath(profile);
  const finish = () => {
    save({ ...profile, completed: true, recommendationId: recommendation.id, updatedAt: new Date().toISOString() });
    setMode(profile.experienceLevel === "advanced" ? "advanced" : "beginner");
  ...
  return <div className="page onboarding-page">
    <section className="onboarding-card" aria-labelledby="onboarding-title">
      <header>
        <span className="eyebrow">{he ? `שלב ${step} מתוך 5` : `Step ${step} of 5`}</span>
        <progress max={5} value={step} aria-label={he ? "התקדמות ההיכרות" : "Onboarding progress"} />
        <h1 id="onboarding-title">{he ? "התאמת האקדמיה אליך" : "Personalize your Academy"}</h1>
        <p>{he ? "חמש החלטות קצרות. תחומי העניין אינם מסומנים מראש, ואפשר לדלג בכל שלב." : "Five short decisions. No interests are preselected, and you can skip at any step."}</p>
      </header>
      ...
      {step === 3 && <fieldset><legend>{he ? "אילו תחומים מעניינים אותך? (לא חובה)" : "Which topics interest you? (optional)"}</legend><div className="onboarding-options">{interests.map((interest) => <label key={interest.id}><input type="checkbox" checked={profile.interests.includes(interest.id)} onChange={() => setProfile((current) => ({ ...current, interests: current.interests.includes(interest.id) ? current.interests.filter((item) => item !== interest.id) : [...current.interests, interest.id] }))} />{interest[language]}</label>)}</div></fieldset>}
      {step === 4 && <fieldset><legend>{he ? "מאיפה תרצה להתחיל?" : "Where would you like to start?"}</legend><div className="onboarding-options">{startingAreas.map((area) => <label key={area.id}><input type="radio" name="starting-area" checked={startingArea === area.id} onChange={() => setStartingArea(area.id)} />{area[language]}</label>)}</div></fieldset>}
      {step === 5 && <div><h2>{he ? "המסלול המומלץ עבורך" : "Your recommended starting path"}</h2><article className="recommendation-card"><span className="status-badge">{recommendation.kind}</span><h3>{recommendation.title[language]}</h3><p>{recommendation.description[language]}</p></article><p>{he ? "אפשר לשנות את הבחירות ואת ההמלצה מאוחר יותר דרך הפרופיל." : "You can change your choices and recommendation later from Profile."}</p></div>}
      <footer className="onboarding-actions">
        {step > 1 && <button type="button" onClick={() => setStep((current) => current - 1)}>{he ? "הקודם" : "Previous"}</button>}
        {step < 5 ? <button className="primary-button" type="button" onClick={() => setStep((current) => current + 1)}>{he ? "הבא" : "Next"}</button> : <button className="primary-button" type="button" onClick={finish}>{he ? "פתיחת לוח הבקרה" : "Open Dashboard"}</button>}
