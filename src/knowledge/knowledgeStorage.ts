...
export function validateKnowledgeInput(input: KnowledgeInput): string[] { const bytes = new TextEncoder().encode(input.content).length; const errors: string[] = []; if (!input.title.trim()) errors.push("titleRequired"); if (!input.content.trim()) errors.push("contentRequired"); if (bytes > MAX_KNOWLEDGE_BYTES) errors.push("tooLarge"); if (!["text", "markdown", "json", "csv", "note"].includes(input.contentType)) errors.push("unsupportedType"); if (input.contentType === "json") try { JSON.parse(input.content); } catch { errors.push("malformedJson"); } return errors; }
export function createKnowledgeDocument(input: KnowledgeInput): KnowledgeDocument { const errors = validateKnowledgeInput(input); if (errors.length) throw new Error(errors.join(",")); const now = new Date().toISOString(); const size = new TextEncoder().encode(input.content).length; return { ...input, filename: input.filename ? sanitizeFilename(input.filename) : undefined, id: crypto.randomUUID(), size, createdAt: now, updatedAt: now, version: 1, hash: hash(input.content), status: "ready" }; }
export function searchKnowledge(documents: readonly KnowledgeDocument[], query: string) { const normalized = query.trim().toLocaleLowerCase(); if (!normalized) return documents; return documents.filter((document) => `${document.title} ${document.description} ${document.content} ${document.tags.join(" ")} ${document.source}`.toLocaleLowerCase().includes(normalized)); }
