...
async function main() {
  ...
  try {
    ...
    try {
      ...
      await page.evaluate(() => {
        window.sessionStorage.setItem("shabis-ai-academy-demo-session", "active");
      });
      ...
      await writeFile(path.join(outDir, `authenticated-${device}.json`), JSON.stringify(flowResult, null, 2));
      ...
      for (const step of flowResult.steps) {
        ...
        const bestPractices = step.lhr.categories["best-practices"]?.score ?? null;
        const seo = step.lhr.categories.seo?.score ?? null;
        const pass =
          (performance === null || performance >= deviceThresholds.performance) &&
          (accessibility === null || accessibility >= deviceThresholds.accessibility) &&
          (bestPractices === null || bestPractices >= deviceThresholds.bestPractices);
        if (!pass) allPassed = false;
        const route = new URL(step.lhr.requestedUrl).pathname;
        summaries.push({ route, device, performance, accessibility, bestPractices, seo, pass });
        console.log(
          `[${device}] ${step.name}: performance=${performance} accessibility=${accessibility} bestPractices=${bestPractices} seo=${seo} -> ${pass ? "PASS" : "FAIL"}`,
        );
      }

      await writeFile(path.join(outDir, `authenticated-${device}-summary.json`), JSON.stringify(summaries, null, 2));

      if (!allPassed) {
        console.error(`Lighthouse authenticated flow (${device}) failed one or more thresholds.`);
        process.exitCode = 1;
      }
    } finally {
      await browser.close();
