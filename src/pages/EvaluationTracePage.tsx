...
export function EvaluationTracePage() {
  ...
  const markdown = events.map((event) => `- ${event.timestamp} · ${event.eventType} · ${event.summary[language]} · ${event.metadata.gateStatus ?? "INFO"}`).join("\n");
  ...
  return (
    <div className="page evaluation-page" data-testid="evaluation-trace">
      ...
      <TraceTimeline language={language} events={events} />
      <section className="evaluation-panel evaluation-export-panel" aria-labelledby="trace-export-heading">
        ...
        <div className="evaluation-run-actions">
          <button type="button" onClick={() => downloadText("evaluation-trace.json", JSON.stringify(events, null, 2), "application/json")}>{he ? "הורדת JSON" : "Download JSON"}</button>
          <button type="button" onClick={() => downloadText("evaluation-trace.md", `# ${experiment.name}\n\n${markdown}\n`, "text/markdown")}>{he ? "הורדת Markdown" : "Download Markdown"}</button>
