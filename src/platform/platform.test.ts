...
describe("beta platform data", () => {
  it("reports honest typed roadmap states", () => { expect(roadmapItems.some((item) => item.status === "completed" && item.version === "1.2.0-beta.1")).toBe(true); expect(roadmapItems.filter((item) => item.status === "planned").length).toBeGreaterThan(0); });
