...
export function AssistantSidebar() {
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event: globalThis.KeyboardEvent) => { if (event.key === "Escape") setMode("collapsed"); };
    const onBack = () => setMode("collapsed");
    window.addEventListener("keydown", onKeyDown); window.addEventListener("popstate", onBack);
    return () => { window.removeEventListener("keydown", onKeyDown); window.removeEventListener("popstate", onBack); };
  }, [mode, setMode]);
