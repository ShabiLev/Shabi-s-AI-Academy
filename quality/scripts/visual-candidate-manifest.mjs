...
function buildManifest() {
  ...
  writeFileSync(outputPath, JSON.stringify({
    ...
    headSha: process.env.EXPECTED_HEAD_SHA ?? process.env.GITHUB_SHA ?? "unknown",
    baseSha: process.env.BASE_SHA ?? "unknown",
    mergeCommitSha: process.env.MERGE_SHA ?? "unknown",
    workflowRunId: process.env.GITHUB_RUN_ID ?? "local",
    playwrightVersion: process.env.PLAYWRIGHT_VERSION ?? "unknown",
    browserRevision: process.env.BROWSER_REVISION ?? "unknown",
    linuxImageVersion: process.env.LINUX_IMAGE_VERSION ?? "unknown",
