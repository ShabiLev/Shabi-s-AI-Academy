...
export function PageIntroduction() {
  ...
  return <><section className="page-introduction" aria-labelledby="page-context-title">
    ...
    <div className="page-introduction-row">
      <div>
        <p className="eyebrow">{localize(getAreaLabel(page.area), language)}</p>
        {page.id === "help"
          ? <h1 className="page-context-title" id="page-context-title">{localize(page.title, language)}</h1>
          : <p className="page-context-title" id="page-context-title">{localize(page.title, language)}</p>}
        <p>{localize(page.summary, language)}</p>
        {page.glossaryTerms.length > 0 && <div className="page-glossary-terms" aria-label={language === "he" ? "מונחים קשורים" : "Related terms"}>
          {page.glossaryTerms.map((termId) => <GlossaryTrigger key={termId} termId={termId} />)}
        </div>}
      </div>
